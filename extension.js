...
function activate(context) {
    console.log('Java Refactoring Analyzer is now active');
    ...
    const analyzeCommand = vscode.commands.registerCommand('java-refactoring-analyzer.analyze', function () {
        ...
        try {
            const metrics = analyzeJavaCode(code, fileName);
            showMetricsPanel(metrics, fileName);
    ...
    const toggleDecorationsCommand = vscode.commands.registerCommand('java-refactoring-analyzer.toggleDecorations', function () {
        ...
        if (decorationsEnabled) {
            ...
            vscode.window.showInformationMessage('Real-time decorations disabled');
            clearAllDecorations();
        }
    });
    ...
    vscode.workspace.onDidChangeTextDocument(event => {
        if (activeEditor && event.document === activeEditor.document && event.document.languageId === 'java') {
            triggerUpdateDecorations();
        }
    }, null, context.subscriptions);

    // Initial decoration if Java file is already open
    if (activeEditor && activeEditor.document.languageId === 'java') {
        triggerUpdateDecorations();
    }

    context.subscriptions.push(analyzeCommand, toggleDecorationsCommand);
}

function createDecorationTypes() {
    // Method decorations - good (green)
    ...
    // Method decorations - warning (yellow/orange)
...
function analyzeForDecorations(code, methodLocThreshold, classMethodThreshold) {
    ...
    allTypeDeclarations.forEach(typeDecl => {
        // Get class/interface name location
